...
# Mudflat Knowledge v2 —「刊物排印」(The Literary Gazette) 规范
...
- **章节头（Content header）**：眉标 mono 11px/0.2em 琥珀（当前刊物/总索引/星标专辑/检索结果）+ 衬线 30px 标题 + 衬线斜体作者 + mono 计数 chips（总计一枚墨底反白）+ 封面 80×110 墨框版框（内衬 3px 白边）+ 40px 方章新建钮（hover 反白）。
...
- **清样（Review）**：衬线 22px/0.3em「清样 · 翻牌回顾」居中；剩余张数 mono 眉标 + 衬线计数；进度为 6px 连续墨条（当前段染金、余段纸板槽）；卡牌 2px 墨边 + 内衬栏线框，背面衬线编目大字 88px；翻面 rotateY 420ms，飞出 240ms。
- **版权页（Settings）**：衬线 28px 标题 + 琥珀下划；三个分区以栏线起头，mono 标题「一、二、三、」；Key 输入 = 墨框直角 + 衬线 15px + focus 金圈。
